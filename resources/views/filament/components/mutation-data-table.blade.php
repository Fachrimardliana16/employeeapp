...
@if($mutations->isEmpty())
    <div class="text-sm text-gray-500 py-4 text-center border rounded-xl dark:border-white/10">Belum ada histori mutasi.</div>
@else
                    ...
                    @foreach ($mutations as $mutation)
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/5 transition duration-75">
                            ...
                            <td class="px-4 py-4 text-sm text-gray-500 dark:text-gray-400 align-top">
                                <div class="font-medium text-gray-900 dark:text-gray-100 text-sm">{{ $mutation->oldPosition->name ?? '-' }}</div>
                            ...
                            <td class="px-4 py-4 text-sm text-gray-500 dark:text-gray-400 align-top">
                                <div class="font-medium text-gray-900 dark:text-gray-100 text-sm">{{ $mutation->newPosition->name ?? '-' }}</div>
                                <div class="text-xs mt-0.5">
